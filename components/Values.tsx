...
export default function Values() {
  ...
  const items = [
    ...
    { k: 'integrity', title: t('items.integrity.title'), desc: t('items.integrity.desc') },
    { k: 'collab',    title: t('items.collab.title'),    desc: t('items.collab.desc') },
    { k: 'commit',    title: t('items.commit.title'),    desc: t('items.commit.desc') },
  ];
  ...
  return (
    <section id="valores" aria-labelledby="values-title" className="relative bg-slate-50/70 py-16 sm:py-24">
      <div className="container max-w-7xl px-6">
        ...
        <ol className="mt-10 grid gap-x-10 gap-y-12 sm:grid-cols-2 lg:grid-cols-3 mx-auto max-w-6xl">
          {items.map((it, i) => (
            <li key={it.k} className="w-full max-w-md mx-auto">
              <div className="flex items-start gap-4">
                <span className="inline-flex h-9 w-9 shrink-0 items-center justify-center rounded-full bg-slate-900 text-white text-sm font-semibold">
                  {i + 1}
                </span>
                <div>
